<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Rekapan Absensi Kelas</h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900" x-data="{ range: '{{ old('range', $inputs['range'] ?? 'this_week') }}' }">
                    ...
                    <form action="{{ route('walikelas.rekap.index') }}" method="GET" class="pb-4 mb-6 border-b">
                        ...
                        <div class="flex items-center space-x-4 mt-6">
                            ...
                            @if (!empty($rekapData))
                                <a href="{{ route('walikelas.rekap.export', $inputs) }}"
                    ...
                    @if (request()->has('filter'))
                        @if (!empty($rekapData))
                            <div class="mt-6">
                                ...
                                <div class="text-sm text-gray-600 mt-1 mb-4 space-y-1">
                                    <p><strong>Kelas:</strong>
                                        {{ $kelasList->firstWhere('id', $inputs['kelas_id'])->nama_kelas ?? '-' }}</p>
                                    <p><strong>Mata Pelajaran:</strong>
                                        {{ $mapels->firstWhere('id', $inputs['mapel_id'])->nama_mapel ?? '-' }}</p>
                                    <p><strong>Periode:</strong>
                                        @if (($inputs['range'] ?? 'this_week') == 'custom')
                                            {{ \Carbon\Carbon::parse($inputs['start_date'])->format('d M Y') }} s/d
                                            {{ \Carbon\Carbon::parse($inputs['end_date'])->format('d M Y') }}
                                        @elseif (($inputs['range'] ?? 'this_week') == 'this_month')
                                            Bulan Ini
                                        @else
                                            Minggu Ini
                                        @endif
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            @forelse ($rekapData as $data)
                                                <tr>
                                                    ...
                                                    <td
                                                        class="px-3 py-2 whitespace-nowrap text-sm text-gray-500 border">
                                                        {{ $data['nis'] }}</td>
                                                    @foreach ($data['absensi'] as $status)
                                                        <td
                                                            class="px-3 py-2 whitespace-nowrap text-sm text-center border">
                                                            {{ $status }}</td>
                                                    @endforeach
                                                </tr>
                                            @empty
